  ...
  const submit = async (data) => {
    ...
    try {
      const response = await axios.post(
        `${import.meta.env.VITE_BASE_URL}/api/v1/captian/login`,
        data,
  ...
  return (
    <div className="p-4 h-screen flex flex-col justify-end">
      ...
      <div>
        <p className="text-sm">
          Join a fleet?{" "}
          <Link className="text-blue-900 text-sm" to="/captian-signup">
      ...
      <div className="">
        <Link
          className="bg-purple-800 text-white w-full py-2 rounded mt-4 flex items-center justify-center"
          to="/login"
        >
          Login as user
        </Link>
