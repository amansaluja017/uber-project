...
function CaptianSignup() {
  ...
  const submit = async (data) => {
    setError('')
    try {
      const response = await axios.post(`${import.meta.env.VITE_BASE_URL}/api/v1/captian/register`, data);
      ...
      <div className='inline-block pl-5 pb-12'>
        ...
        <form onSubmit={handleSubmit(submit)} className='p-7 flex flex-col justify-around -mt-[5rem] gap-2'>
          <div className='flex flex-col'>
            <Input classNmae='h-9 -mt-3' label='Enter your first name' type='text' placeholder='First name' {...register('firstName', {required: true})} ></Input>
          </div>
          <div className='mt-3 flex flex-col'>
            <Input classNmae='h-9 -mt-3' label='Enter you last name' type='text' placeholder='Last name' {...register('lastName')}></Input>
          </div>
          <div className='mt-3 flex flex-col'>
            <Input classNmae='h-9 -mt-3' label='Enter your email' type='email' placeholder='example@example.com' {...register('email', {required: true})}></Input>
          </div>
          <div className='mt-3 flex flex-col'>
            <Input classNmae='h-9 -mt-3' label='Enter your Password' type='password' placeholder='Password' {...register('password', {required: true})}></Input>
          </div>
          ...
          <div className='py-2'>
            ...
            <div className='mt-3 grid grid-cols-2 gap-2' {...register('vehicle', {required: true})}>
              <Input classNmae='h-9' type='text' placeholder='color' {...register('vehicle.color', {required: true})}></Input>
              <Input classNmae='h-9' type='text' placeholder='plate' {...register('vehicle.plate', {required: true})}></Input>
              <Input classNmae='h-9' type='text' placeholder='capicity' {...register('vehicle.capicity', {required: true})}></Input>
