...
function CaptianRide() {
  ...
  const endRide = async () => {
    try {
      await axios.post(`${import.meta.env.VITE_BASE_URL}/api/v1/rides/end-ride`, {
        rideId: rideData._id
      }, {
  ...
  return (
    <div className='h-screen'>
      ...
      <div ref={finishRidePanelRef} className='h-full bg-white flex top-[20%] z-11 fixed translate-y-full flex-col w-full'>
        <h5 onClick={() => { setFinishRidePanel(false) }} className='text-center font-bold text-gray-300 text-2xl'><i className="ri-arrow-down-wide-fill"></i></h5>
        <h3 className='absolute top-0 text-xl font-bold mt-8 p-4'>Finish this Ride </h3>
        <FinishRide setFinishRidePanel={setFinishRidePanel} endRide={endRide} setRideEndedPanel={setRideEndedPanel} rideData={rideData} />
      </div>

      <div ref={rideEndedPanelRef} className='h-full translate-y-full bg-white flex z-11 fixed top-[75%] flex-col w-full'>
        <RideEnded setRideEndedPanel={setRideEndedPanel} setPaymentReceivePanel={setPaymentReceivePanel} endRide={endRide} />
      </div>

      <div ref={paymentReceivePanelRef} className='h-full bg-green-400 translate-y-full flex z-11 fixed top-[75%] justify-center py-5 w-full'>
        <PaymentReceive />
      </div>
